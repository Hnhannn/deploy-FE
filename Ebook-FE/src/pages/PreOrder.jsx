...
function PreOrder() {
  ...
  const handleQuantityChange = (action) => {
    setQuantity((prevQuantity) => {
      let newQuantity = action === "+" ? prevQuantity + 1 : prevQuantity - 1;
      if (newQuantity < 1) {
        newQuantity = 1;
      }
      return newQuantity;
  ...
  const totalPrice = (bookDetails?.price || 0) * quantity + shippingFee;
  ...
  const handlePayment = async () => {
    if (!selectedPaymentMethod) {
      toast.error(
        "Vui lòng chọn phương thức thanh toán QR CODE hoặc Thanh toán khi nhận hàng!"
      );
      return;
    }
    ...
    if (isFormValid) {
      ...
      try {
        if (selectedPaymentMethod === "qr") {
          const data = await BookPaymentService.handlePayment(totalPrice);
          if (data.redirectUrl) {
            // Lưu thông tin tạm thời vào localStorage
            ...
            localStorage.setItem("bookIDs", bookID);
            localStorage.setItem("quantity", quantity);
            localStorage.setItem("price", bookDetails.price);
          ...
          <div className="flex-1">
            ...
            <div className="w-full pt-[3.75rem]">
              ...
              <table className="w-full bg-c1c rounded-xl animate-[appearing_.3s_linear_alternate]">
                ...
                <tbody>
                  <tr className="text-white-50 font-medium border-t border-[hsla(0,_0%,_100%,_.1)]">
                    <td className="py-4">
                      <div className="flex items-center gap-4 px-6">
                        ...
                        <div>
                          <p className="t-ellipsis-2">{bookDetails.title}</p>
                    ...
                    <td className="text-center">
                      {bookDetails.price.toLocaleString("vi-VN")}
                    </td>
                    <td>
                      <div className="w-full flex items-center justify-center">
                        <div className="flex items-center text-white-50 font-normal">
                          ...
                          <input
                            ...
                            min={1}
                            max={20}
                            className="pre-order__number-input w-15 h-9 border-y border-[rgba(255,255,255,0.2)] bg-transparent appearance-none text-center"
                            placeholder={1}
                          />
                          <button
                            onClick={() => handleQuantityChange("+")}
          ...
          <div className="w-[27rem] h-fit bg-c1c rounded-xl flex flex-col divide-y divide-white-overlay sticky right-0 top-[153px] text-white-default font-medium">
            <div className="px-6 py-4">
              <div className="flex justify-between">
                <p>Thành tiền</p>
                <p>
                  {bookDetails?.price * quantity > 0
                    ? `${(bookDetails?.price * quantity).toLocaleString(
                        "vi-VN"
                      )} VND`
                    : "Chưa tính thành tiền"}
                </p>
              </div>
              <div className="flex justify-between">
